...
import { processDataImperative, processData, pullAndCompute } from '../src/ex4-ex5.js'
...
describe('ex. 4', function () {
  ...
  for (const implementation of [processDataImperative, processData]) {
...
describe('ex. 5', function () {
  describe('pullAndCompute()', function () {
    it('should compute various statistics about contributors', async function () {
      const stats = await pullAndCompute()
      expect(stats["firstUniqueProject"]).to.eq('abdera')
      expect(stats["uniqueContributorsCount"]).to.eq(4595)
      expect(stats["averageLength"].toFixed(2)).to.eq('14.13')
      expect(stats["mostContributor"]).to.eq('Jim Jagielski')
      expect(stats["top10ProjectsName"]).to.deep.eq([
        'incubator',
